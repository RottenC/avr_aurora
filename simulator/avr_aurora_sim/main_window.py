...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.sim = Simulation(); self.paused = False; self.speed = 1.0
        ...
        self.power_blink_half = QSpinBox(); self.power_blink_half.setRange(100, 5000); self.power_blink_half.setSuffix(" ms half-period"); self.power_blink_half.setValue(500); self.power_blink_half.valueChanged.connect(lambda v: setattr(self.sim.power_led_generator, "half_period_ms", v)); left.addWidget(self.power_blink_half)
        ...
        self.filter = QLineEdit(); self.filter.setPlaceholderText("Diagnostic operation filter"); self.filter.textChanged.connect(self._refresh_diagnostics); left.addWidget(self.filter)
        ...
        self.timeline = TimelineWidget(); right.addWidget(QLabel("Timeline (last 15 s simulated)")); right.addWidget(self.timeline)
        self.status = QLabel(); self.status.setAlignment(Qt.AlignTop); right.addWidget(self.status)
        self.diag_table = QTableWidget(0, 6); self.diag_table.setHorizontalHeaderLabels(["time", "frame", "operation", "label", "inputs", "result"]); right.addWidget(self.diag_table)
    ...
    def _power_source_changed(self, text):
        self.sim.set_power_led_mode(PowerLedSourceMode(text)); self.power_led.setEnabled(PowerLedSourceMode(text) is PowerLedSourceMode.MANUAL)
    def _mode_changed(self, text):
        self.sim.set_hdd_mode(HddMode(text)); self._sync_param_controls(); self.hdd_led.setEnabled(HddMode(text) is HddMode.MANUAL)
    ...
    def _fps_changed(self, fps):
        interval = max(1, round(1000 / fps)); self.sim.config.target_fps = fps; self.sim.config.frame_interval_ms = interval; self.timer.setInterval(interval)
    ...
    def _restart(self): self.sim.restart(); self.timeline.clear(); self._sync_controls_from_model(); self._refresh()
    def _tick(self, force):
        ...
        try:
            ...
            self.paused = True; self.pause.setText("Resume"); QMessageBox.critical(self, "Strict diagnostic", str(error)); self._refresh_diagnostics()
    ...
    def _sync_controls_from_model(self):
        ...
        for widget, value in [(self.power_led, self.sim.inputs.manual_power_led), (self.hdd_led, self.sim.inputs.manual_hdd_led), (self.strip_power, self.sim.inputs.strip_power), (self.strict, self.sim.diagnostics.strict)]:
    ...
    def _refresh(self):
        leds = self.sim.led_buffer.to_list() if self.sim.inputs.strip_power else [(0, 0, 0)] * 56; self.linear.set_leds(leds); self.physical.set_leds(leds)
        s = self.sim.state; i = self.sim.inputs; pc = self.sim.pc_state_machine; ec = self.sim.effect_controller
        self.raw_power.setText(f"Raw Power LED: {s.raw_power_led}"); self.raw_hdd.setText(f"Raw HDD LED: {s.raw_hdd_led}"); self.power.setText(f"Power hold: {pc.hold_duration(s.now_ms)/1000:.2f} / 4.00 s")
        lines = [f"Simulated time: {s.now_ms} ms", f"Frame: {s.frame_number}", f"Target FPS: {self.sim.config.target_fps} ({self.sim.config.frame_interval_ms} ms)", f"Power LED source: {self.sim.power_led_generator.mode.value}", f"Raw Power LED: {s.raw_power_led}", f"Classified Power LED: {s.power_led_mode.value}", f"PC state: {pc.state.value}", f"Transition: {ec.current.value}", f"Transition elapsed/duration/progress: {ec.elapsed(s.now_ms)} / {ec.duration()} ms / {ec.progress8(s.now_ms)}", f"Manual Power LED: {i.manual_power_led}", f"Manual HDD LED: {i.manual_hdd_led}", f"Raw HDD LED: {s.raw_hdd_led}", f"Active HDD edges this frame: {s.active_edges_this_frame}", f"Pending active edges: {self.sim._pending_edge_count}", f"Pending HDD milliseconds: {s.hdd_pending_ms}", f"Smoothed HDD activity: {s.hdd_activity}/128", f"Power hold duration: {pc.hold_duration(s.now_ms)} ms", f"Forced shutdown latched: {pc.forced_latched}", f"Starting elapsed: {pc.starting_elapsed(s.now_ms)} ms", f"Await-shutdown elapsed: {pc.await_shutdown_elapsed(s.now_ms)} ms", f"Strip power: {i.strip_power}", f"Current random seed: {s.random_seed}", f"Preview elapsed/duration/progress: {self.sim.preview_elapsed_ms()} / {self.sim.preview_duration_ms()} ms / {self.sim.preview_progress8()}", "Diagnostics:"]
        lines.extend(f"  {key}: {value}" for key, value in sorted(self.sim.diagnostics.counters.items())); self.status.setText("\n".join(lines)); self._refresh_diagnostics()
    def _refresh_diagnostics(self):
        filt = self.filter.text() if hasattr(self, "filter") else ""; events = [e for e in self.sim.diagnostics.events if not filt or filt in e.operation]
        self.diag_table.setRowCount(len(events))
        for row, event in enumerate(events):
            for col, value in enumerate([event.now_ms, event.frame_number, event.operation, event.label, event.inputs, event.result]):
                self.diag_table.setItem(row, col, QTableWidgetItem(str(value)))
