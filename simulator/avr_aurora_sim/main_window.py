...
class MainWindow(QMainWindow):
    ...
    def _refresh(self):
        ...
        s = self.sim.state; i = self.sim.inputs; pc = self.sim.pc_state_machine; ec = self.sim.effect_controller; ctx = s.last_context or self.sim.context()
        ...
        lines = [f"Simulated time: {s.now_ms} ms", f"Frame: {s.frame_number}", f"Target FPS: {self.sim.config.target_fps} ({self.sim.config.frame_interval_ms} ms)", f"Power LED source: {self.sim.power_led_generator.mode.value}", f"Raw Power LED: {s.raw_power_led}", f"Classified Power LED: {s.power_led_mode.value}", f"PC state: {pc.state.value}", "Controller:", f"  current transition: {ec.current.value}", f"  duration/lifetime: {controller_lifetime}", "Visual context:", f"  elapsed: {ctx.transition_elapsed_ms} ms", f"  duration: {ctx.transition_duration_ms} ms", f"  progress: {ctx.transition_progress}", f"Manual Power LED: {i.manual_power_led}", f"Manual HDD LED: {i.manual_hdd_led}", f"Raw HDD LED: {s.raw_hdd_led}", f"Active HDD edges this frame: {s.active_edges_this_frame}", f"Pending active edges: {self.sim._pending_edge_count}", f"Pending HDD milliseconds: {s.hdd_pending_ms}", f"Smoothed HDD activity: {s.hdd_activity}/128", f"Power hold duration: {pc.hold_duration(s.now_ms)} ms", f"Forced shutdown latched: {pc.forced_latched}", f"Starting elapsed: {pc.starting_elapsed(s.now_ms)} ms", f"Await-shutdown elapsed: {pc.await_shutdown_elapsed(s.now_ms)} ms", f"Strip power: {i.strip_power}", f"Current random seed: {s.random_seed}", f"Preview elapsed/duration/progress: {self.sim.preview_elapsed_ms()} / {self.sim.preview_duration_ms()} ms / {self.sim.preview_progress8()}", "Diagnostics:"]
    ...
    def _refresh_diagnostics(self, force=False):
        revision = len(self.sim.diagnostics.events)
