...
void EffectController::update(PcStateMachine &pc, bool stripPowerPresent, uint32_t nowMs) {
  ...
  if (current_ == TransitionEffect::Startup && age >= Config::StartupDurationMs) {
    current_=TransitionEffect::None;
    pc.markStartupComplete();
  } else if (current_ == TransitionEffect::Shutdown && age >= Config::ShutdownDurationMs) {
    current_=TransitionEffect::None;
  } else if (current_ == TransitionEffect::Reset && age >= Config::ResetDurationMs) {
    current_=TransitionEffect::None;
  }
}
