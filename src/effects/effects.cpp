...
void renderAurora(CRGB *leds, uint8_t count, uint32_t nowMs, uint8_t hddActivity){
  ...
  const uint8_t bright = Config::AuroraBaseBrightness + (Config::AuroraHddAffectsBrightness ? scale8(hddActivity, Config::AuroraHddBrightnessBoost) : 0);
  for (uint8_t i=0;i<count;++i){ const uint8_t wave = sin8(i * 9 + (nowMs / 16) * speed / 16); leds[i] = CHSV(96 + wave / 5, 190, scale8(wave, bright)); }
}
void renderSleep(CRGB *leds, uint8_t count, uint32_t nowMs){
  ...
  leds[p1] = CHSV(160, 180, b); leds[p2] = CHSV(176, 160, b / 2);
}
void renderTransition(CRGB *leds, uint8_t count, TransitionEffect effect, uint32_t startedAt, uint32_t nowMs){
  ...
  if (effect == TransitionEffect::Startup) {
    ...
    const uint8_t b = age < Config::ForcedFlashAtMs ? map(age, 0, Config::ForcedFlashAtMs, 10, 160) : map(min(age, static_cast<uint32_t>(Config::PowerHoldForcedMs)), Config::ForcedFlashAtMs, Config::PowerHoldForcedMs, 160, 0);
