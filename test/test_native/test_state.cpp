...
PcStateInputs makeInputs(PowerLedMode mode) {
  return {false, false, false, false, mode, false};
}
...
void test_auto_01_off_continuously_reconciles_to_observed_state() {
  ...
  PcStateInputs sleeping = makeInputs(PowerLedMode::Blinking);
...
void test_auto_03_startup_requires_animation_and_power_confirmation() {
  ...
  inputs.startupAnimationFinished = true;
  pc.update(inputs, 300);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
}

void test_auto_04_reset_press_requests_temporary_animation() {
  PcStateMachine pc = makePcStateMachine();
  reconcileToRunning(pc);

  PcStateInputs inputs = makeInputs(PowerLedMode::On);
  inputs.resetButtonPressed = true;
  PcStateEvents events = pc.update(inputs, 100);
  TEST_ASSERT_TRUE(events.requestReset);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running),
                          static_cast<uint8_t>(pc.state()));

  inputs.resetButtonPressed = false;
  events = pc.update(inputs, 101);
  TEST_ASSERT_FALSE(events.requestReset);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running),
                          static_cast<uint8_t>(pc.state()));
}
...
void test_auto_06_startup_times_out_or_reconciles_to_sleep() {
  ...
  pc.update(inputs, 100);
  inputs.powerButtonPressed = false;
  pc.update(inputs, 100 + Config::StartingTimeoutMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Off), static_cast<uint8_t>(pc.state()));
...
void test_auto_08_forced_hold_latches_and_release_uses_no_normal_shutdown() {
  ...
  PcStateInputs inputs = makeInputs(PowerLedMode::On);
  inputs.powerButton = true;
  pc.update(inputs, 100 + Config::PowerHoldForcedMs);
  TEST_ASSERT_TRUE(pc.forcedLatched());

  inputs.powerButton = false;
  inputs.powerButtonReleased = true;
  pc.update(inputs, 200 + Config::PowerHoldForcedMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::AwaitShutdown), static_cast<uint8_t>(pc.state()));
}
...
void test_auto_17_forced_shutdown_boundary() {
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    PcStateInputs inputs = makeInputs(PowerLedMode::On);
    inputs.powerButtonReleased = true;
    pc.update(inputs, 100 + releases[i]);
    TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::AwaitShutdown), static_cast<uint8_t>(pc.state()));
...
void test_auto_18_millis_overflow_state_and_effect_timing() {
  ...
  shutdown.update(makeInputs(PowerLedMode::On), Config::AwaitShutdownTimeoutMs - 21);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(shutdown.state()));

}

void test_auto_19_power_led_on_reconciles_to_running() {
  PcStateMachine pc = makePcStateMachine();
  pc.update(makeInputs(PowerLedMode::On), 10);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
  pc.update(makeInputs(PowerLedMode::On), 20);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
}

void test_auto_21_await_shutdown_ignores_blinking_until_timeout() {
  PcStateMachine pc = makePcStateMachine();
  reconcileToRunning(pc);
  beginPowerHold(pc, 100);
  PcStateInputs release = makeInputs(PowerLedMode::On);
  ...
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::AwaitShutdown), static_cast<uint8_t>(pc.state()));
  pc.update(makeInputs(PowerLedMode::Blinking), 200 + Config::AwaitShutdownTimeoutMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
  pc.update(makeInputs(PowerLedMode::Blinking), 300 + Config::AwaitShutdownTimeoutMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Sleeping), static_cast<uint8_t>(pc.state()));
}
...
void test_auto_26_startup_animation_finish_waits_for_power_led() {
  ...
  inputs.powerButtonPressed = false;
  inputs.startupAnimationFinished = true;
  pc.update(inputs, 100 + Config::StartupDurationMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Starting), static_cast<uint8_t>(pc.state()));
  pc.update(inputs, 100 + Config::StartupDurationMs + 1000);
  inputs.powerMode = PowerLedMode::On;
  pc.update(inputs, 100 + Config::StartupDurationMs + 2000);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
}
...
void test_auto_28_await_shutdown_timeout_returns_running_and_reconciles_off() {
  PcStateMachine pc = makePcStateMachine();
  ...
  pc.update(release, 200);
  pc.update(makeInputs(PowerLedMode::On), 200 + Config::AwaitShutdownTimeoutMs - 1);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::AwaitShutdown), static_cast<uint8_t>(pc.state()));
  pc.update(makeInputs(PowerLedMode::On), 200 + Config::AwaitShutdownTimeoutMs);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Running), static_cast<uint8_t>(pc.state()));
  pc.update(makeInputs(PowerLedMode::Off), 200 + Config::AwaitShutdownTimeoutMs + 1);
  TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(PcState::Off), static_cast<uint8_t>(pc.state()));
}
...
int main(int, char **) {
  ...
  RUN_TEST(test_auto_04_reset_press_requests_temporary_animation);
  ...
  RUN_TEST(test_auto_19_power_led_on_reconciles_to_running);
  RUN_TEST(test_auto_21_await_shutdown_ignores_blinking_until_timeout);
  ...
  RUN_TEST(test_auto_28_await_shutdown_timeout_returns_running_and_reconciles_off);
